...
<section>
    <div class="m-5">
        <a href="{{url('admin/adminDashboard/pets/index')}}" class="font-bold">
            Pets List
        </a>
    </div>
                ...
                <div id="addPetsModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
                    <div class="relative p-4 w-full max-w-4xl h-full md:h-auto">
                        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                            ...
                            <!-- Modal body -->
                            @include('admin.adminDashboard.pets.create')
                        </div>
                    </div>
                </div>
            </div>            
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-300 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3 text-center">Vaccine</th>
                    ...
                    <tbody>
                        @foreach($pets as $pet )
                            <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                {{-- Id --}}
                                <td scope="row" class="w-4 px-4 py-3 text-center font-medium text-gray-900">
                                    {{ $loop->iteration }}
                                </td>
                                ...
                                {{-- Image --}}
                                <th scope="row" class="flex items-center justify-center px-4 py-2 font-medium text-gray-900 dark:text-white">
                                    <img src="{{ asset('/images/' . $pet->images) }}" alt="pro-image" class="w-10 h-10 object-cover mr-3">
                                </th>

                                {{-- Age --}}
                                <td class="px-4 py-2 text-center font-medium text-gray-900 dark:text-white">{{ $pet->age}}</td>

                                {{-- Sex --}}
                                <td class="px-4 py-2 text-center font-medium text-gray-900 dark:text-white">{{ $pet->sex }}</td>

                                {{-- Vaccine --}}
                                <td class="px-4 py-2 text-center font-medium text-gray-900 dark:text-white">{{ $pet->vaccine }}</td>

                                {{-- Vaccine --}}
                                <td class="px-4 py-2 text-start font-medium text-gray-900 dark:text-white w-[240px]"><p class="line-clamp-2">{{ $pet->content }}</p></td>
                                    ...
                                    <div id="{{$pet->id}}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
                                        <div class="relative p-4 w-full max-w-4xl h-full md:h-auto">
                                            <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                                ...
                                                <!-- Modal body -->
                                                @include('admin.adminDashboard.pets.show')
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-2 text-center">
                                    <button id="{{ $loop->iteration }}" data-dropdown-toggle="{{"edit" . $pet->id}}" class="inline-flex items-center text-sm font-medium text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                                        </svg>
                                    </button>
                                    <div id="{{"edit" . $pet->id}}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                        <ul class="pt-4 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="{{ $loop->iteration }}">
                                            
                                            {{-- delete --}}
                                            <li>
                                                <form method="POST" action="{{ url('/admin/adminDashboard/pets/index/' . $pet->id)}}" accept-charset="UTF-8" style="display:inline" class="block py-4 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                    {{ csrf_field() }}
                                            ...
                                            {{-- edit --}}
                                            <li>
                                                <a href="{{ url('/admin/adminDashboard/pets/update/'. $pet->id)}}" title="Edit Item" class="block text-start py-4 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">

                ...
                <div class="row">
                    <div class="col">
                        <form class="form-group mt-" method="post"
                            action="{{ url('/admin/adminDashboard/petTreatment/update/' . $pet->id) }}" enctype="multipart/form-data">
                            {!! csrf_field() !!}
                            ...
                            <div class="flex gap-4">
                                ...
                                <div class="flex flex-col w-full">
                                    ...
                                    <input value="{{ $pet->address }}" type="text" name="address" id="address" required placeholder="address" class="bg-gray-50 border border-gray-300 py-1.5 px-4 rounded-lg mt-2 text-sm">
                                </div>
                            </div>
                            ...
                            <div class="flex gap-4 mt-5">
                                <div class="flex flex-col w-full">
                                    ...
                                    <input value="{{ $pet->phoneNumber }}" type="number" name="phoneNumber" id="phoneNumber" required placeholder="phoneNumber" class="bg-gray-50 border border-gray-300 py-1.5 px-4 rounded-lg mt-2 text-sm">
                                </div>
    
                                <div class="flex flex-col w-full">
                                    <label for="formGroupExampleInput2">Email</label>
                                    <input value="{{ $pet->email }}" type="email" name="email" id="email" required placeholder="email" class="bg-gray-50 border border-gray-300 py-1.5 px-4 rounded-lg mt-2 text-sm">
                                </div>
                            </div>

                            <div class="flex gap-4 mt-5">
                                <div class="flex flex-col w-full">
                                    <label for="formGroupExampleInput2">openDay</label>
                                    <input value="{{ $pet->openDay }}" type="text" name="openDay" id="openDay" required placeholder="openDay" class="bg-gray-50 border border-gray-300 py-1.5 px-4 rounded-lg mt-2 text-sm">
                                </div>
    
                                <div class="flex flex-col w-full">
                                    <label for="formGroupExampleInput2">TimeOpen</label>
                                    <input value="{{ $pet->timeOpen }}" type="number" name="timeOpen" id="timeOpen" required placeholder="timeOpen" class="bg-gray-50 border border-gray-300 py-1.5 px-4 rounded-lg mt-2 text-sm">
                                </div>
                            </div>

                            <div class="flex flex-col mt-5">
                                <label for="formGroupExampleInput2">Location</label>
                                <textarea class="border border-gray-300 bg-gray-50 mt-2 p-2.5 rounded-lg text-sm" rows="5" name="content" id="content" required placeholder="Content">{{ $pet->location }}</textarea>
                            </div>
                                ...
                                <div class="flex items-center justify-center w-full">
                                    <label for="image" class="flex flex-col items-center justify-center w-full h-[500px] cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                                        <div class="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full relative">
                                            ...
                                            <img src="{{ asset('petGrooming/' . $pet->images) }}" id="image-preview" class="text-white absolute w-full h-full object-cover rounded-lg">

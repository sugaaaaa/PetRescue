<x-app-layout>
    <section>
        {{-- Banner --}}
        <div class="bg-[#F9F6E5] pt-28 pb-10 sm:pt-28 sm:pb-24 text-center">
            <div class="flex justify-center items-center gap-5">
                <h1 class="text-xl sm:text-3xl font-bold border-r-2 border-gray-900 px-5">Pet Shop</h1>
                <a href="/">
                    <h2 class="text-md sm:text-xl">Home</h2>
                </a>
            </div>
              <p class="mt-5 text-sm">You can check the location of pet Treatment nearest you in Cambodia!</p>
            <div class="flex gap-5 justify-center mt-5 text-sm">
                <a href="{{ url('pages/petTreatment') }}" class="px-5 py-1 sm:px-10 sm:py-1.5 shadow-[1px_0px_25px_0px_#cbcbcb] rounded-full bg-white hover:bg-[#EBD8C4]">Pet Treatment</a>
                <a href="{{ url('pages/petPark') }}" class="px-5 py-1 sm:px-10 sm:py-1.5 border border-[#F4C38F] rounded-full hover:bg-[#EBD8C4]">Pet Parks</a>
        ...
        <div class="mt-5 sm:mt-24 max-w-screen-xl mx-auto px-5">
            ...
            <div class="grid lg:grid-cols-2 gap-2 sm:gap-4 mt-2 sm:mt-5 px-4 sm:px-5">
                @foreach($petTreatment as $pet)
                <a href="#" class="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
                    <img class="object-cover w-full rounded-t-lg h-96 md:h-60 md:w-60 md:rounded-none md:rounded-s-lg" src="{{ asset('/PetTreatment/' . $pet->images) }}" alt="">
